...
def welcomepic(pic, user, chatname, id, uname):
    background = Image.open("assets/rswelcm.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp)
    pfp = pfp.resize((835, 839))
    draw = ImageDraw.Draw(background)
    font_large = ImageFont.truetype('assets/ArialReg.ttf', size=65)
    font_small = ImageFont.truetype('assets/ArialReg.ttf', size=60)
